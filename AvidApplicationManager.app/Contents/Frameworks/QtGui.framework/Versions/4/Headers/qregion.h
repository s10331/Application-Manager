...
public:
    ...
    CarbonAPI::RgnHandle toQDRgn() const;
    CarbonAPI::RgnHandle toQDRgnForUpdate_sys() const;
    static QRegion fromQDRgn(CarbonAPI::RgnHandle shape);
#endif
#ifdef QT_MAC_USE_COCOA
    inline HIMutableShapeRef handle(bool unused = false) const
    { Q_UNUSED(unused); return toHIMutableShape(); }
#else
    inline CarbonAPI::RgnHandle handle() const { return handle(false); }
    inline CarbonAPI::RgnHandle handle(bool) const { return toQDRgn(); }
#endif
...
private:
    ...
    struct QRegionData {
        ...
        mutable CarbonAPI::RgnHandle unused; // Here for binary compatibility reasons. ### Qt 5 remove.
#endif
#if defined(Q_WS_QWS) || defined(Q_WS_QPA) || defined(Q_WS_X11) || defined(Q_WS_MAC) || defined(Q_WS_WIN) || defined(Q_OS_SYMBIAN)
